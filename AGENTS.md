...
- Build a pragmatic Laravel app on top of Postgres data, including FAQ data
  migrated or re-ingested from the legacy Supabase-hosted Postgres source.
...
Supabase is legacy infrastructure and an external data source, not an
application dependency or integration target for the Laravel app going forward.
The Laravel app should use Postgres/pgvector directly.

When inspecting, debugging, or migrating data from the legacy self-hosted
Supabase environment, use direct `psql` access rather than the Supabase CLI.
...
legacy Supabase connection fails, verify the current password from the
production Supabase env on the server before falling back to the Supabase SQL
Editor.
